...
- (NSArray *)rankMatchesForQuery:(NSString *)query inStrings:(NSArray *)strings
{
    ...
    for (NSArray *v in strings) {
        ...
        if ([s rangeOfString:[query substringToIndex:3] options:(NSCaseInsensitiveSearch|NSDiacriticInsensitiveSearch)].location != NSNotFound) {
            NSArray *m = [re matchesInString:s options:NSMatchingWithoutAnchoringBounds range:NSMakeRange(0, [s length])];
            [mat addObjectsFromArray:m];
        } else {
            [data removeObject:v];
        }
    }
    ...
    for (NSTextCheckingResult *m in mat) {
        NSInteger groups = [m numberOfRanges];
        NSMutableArray *locations = [[NSMutableArray alloc] initWithCapacity:0];
        for (NSInteger i = 1; i < groups; i++) {
            NSRange end = [m rangeAtIndex:i];
            [locations addObject:[NSNumber numberWithInteger:end.location+end.length]];
        }
        [position addObject:locations];
        locations = nil;
    }
    NSMutableArray *letter_seq = [[NSMutableArray alloc] initWithCapacity:0];
    for (NSArray *pos in position) {
        NSInteger seq = 0;
        for (NSInteger i = 1; i < [pos count]; i++) {
            seq += ([[pos objectAtIndex:i] integerValue] - [[pos objectAtIndex:i - 1] integerValue]) == 1;
        }
...
- (NSArray *)fuzzySearchFor:(NSString *)query withKeyBlock:(NSString *(^)(id))key
{
    ...
    for (NSInteger i = 0; i < [O count]; i++) {
        [O_ insertObject:[[O objectAtIndex:i] objectAtIndex:1] atIndex:i];
    }
